...
import ma.enset.tpja.repository.PatientRepository;
...
public class TpJpaApplication implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		...
		patientRepository.save(new Patient(null,"koubach",new Date(),200,false));
		patientRepository.save(new Patient(null,"fahd",new Date(),1300,true));
		patientRepository.save(new Patient(null,"aatouf",new Date(),2500,true));
		patientRepository.save(new Patient(null,"nada",new Date(),340,true));
       ...
       System.out.println("***************************");
		...
		patientRepository.findAll().forEach(p->{
         ...
         System.out.println("***************************");
         List<Patient> patients3=patientRepository.findByNomContainsAndMalade("f",true);
		 patients3.forEach(p->{
			 System.out.println(p.toString());
		 });
